...
export getPossibleIndices, loadFile, spatialMask, temporalMask, mutualMask, sampleData, plotTrace!, plotState!, recordingMovie, readObsFile, stateInterpolants, generateObservationFileSequence!, spatialMaskCentroid, plotSamplingSquare!, checkObsAgainstMask!
...
const sw = 3.40457#+0.5
const dw = 7.05826#-0.5
...
function generateCompleteInterpolant(states, mutualSpaceMask, mutualTimeMask; dt=2.0, dx=0.06, dz=1.0) # ms and cm, respectively; dz is the thickness of the tissue
	...
	#end
	#return interpolant
	print("interpolant(t,x,y,z):\n\tt in [0,$(dt*(length(it)-1))],\n\tx in [0,$(127*dx)],\n\ty in [0,$(127*dx)],\n\tz in [0,$(dz)]\n\n.")
	return itp
end
...
function square(;theta=0.0, l=3.0, center=[3.5;3.5])
	...
	if theta ≠ 0.0
		@warn "θ ≠ 0 is currently buggy, sorry!"
	end
	if abs(theta) >= pi/2
...
function plotState!(fig, axs, tI, obs, spaceMasks, timeMasks, N, spacemask, timemask; drawColorbar=false)
	
	if drawColorbar  # only draw the colorbar on the first frame
		imm = axs[1].imshow(obs[1][tI,:,:], aspect="equal", vmin=0.0, vmax=1.0, cmap="Oranges", interpolation="none", origin="lower")	
		clb = fig.colorbar(imm, ax=axs[1:2], aspect=40)
	end
	labels = ["EPI", "ENDO"]
	let x = 0:(size(spacemask,1)-1), y = 0:(size(spacemask,2)-1) #x = 0.5:(size(spacemask,1)-0.5), y = 0.5:(size(spacemask,2)-0.5)
		for n=1:2
			axs[n].cla()
			axs[n].imshow(obs[n][tI,:,:], alpha=Float64.(spaceMasks[n]), aspect="equal", vmin=0.0, vmax=1.0, cmap="Oranges", interpolation="nearest", origin="lower")
			axs[n].contour(x, y, Float64.(spacemask), "k", levels=[0.5], linewidths=0.5)
			axs[n].axis("off")
			axs[n].set_xticks([])
			axs[n].set_yticks([])
			axs[n].set_title("$(labels[n])")#, \$ t=\$"*"$(t) ms")
		end
		axs[1].contour(x, y, Float64.(spaceMasks[2]), "k", levels=[0.5], linestyles="dashed", linewidths=0.5)
		axs[2].contour(x, y, Float64.(spaceMasks[1]), "k", levels=[0.5], linestyles="dashed", linewidths=0.5)
	end
	return nothing	
end

function plotSamplingSquare!(fig, axs, corners::Array{T,2}; dx=0.06, dy=0.06) where T <: AbstractFloat
	# Map to continuous coordinates by dividing by the space discretization of the dataset
	axs[1].plot(corners[1,:]./dx, corners[2,:]./dy, "--k", linewidth=1)
	axs[2].plot(corners[1,:]./dx, corners[2,:]./dy, "--k", linewidth=1)
...
function checkObsAgainstMask!(obs, spaceMasks, corners; dx=0.06, dy=0.06, min_x = 0, max_x = 199, min_y = 0, max_y = 199)

	function inMask(x::Int,y::Int,mask)
		return mask[y,x]
	end
	function inMask(x::T, y::T, mask) where T <: AbstractFloat
		return mask[Int(round(y/dy)), Int(round(x/dx))]
	end
	
	lo = length(obs);
	for n in length(obs):-1:1		# start from the end and move to the beginning so we reuse memory
		
		# get real-valued coordinates into state interpolant
		...
		# check whether (x,y) is in the mask by converting to indices of the spacemask
		if (obs[n][2] == 1.0 && !inMask(x,y,spaceMasks[1])) || (obs[n][2] == 50.0 && !inMask(x,y,spaceMasks[2]))
		# if on bottom => EPI sample, if on top => ENDO sample		
				print("Observation $((obs[n][3],obs[n][4])) is outside mask.\n")
				deleteat!(obs, n)
		end
	end
	if length(obs) != lo
		print("Number of observations decreased from $(lo) to $(length(obs)).\n")
	end
	return obs
end

function fillObservations!(obs, stateSplines, t, corners; min_x = 0, max_x = 199, min_y = 0, max_y = 199, err = 0.025, lerp=false)
	...
	for n in 1:length(obs)
		...
		elseif obs[n][2] == 50.0	# if on top => ENDO sample
			...
			#obs[n][6] = err 		# What is the error in the observation here
		elseif lerp
			obs[n][5] = ((50.0-obs[n][2])/(50.0-1.0))*stateSplines[1](t,y,x) + ((obs[n][2]-1.0)/(50.0-1.0))*stateSplines[2](t,y,x)
			#obs[n][6] = 0.25 		# manually increase uncertainty in interior
		end
	end
...
function generateObservationFileSequence!(obs, stateSplines, obsDir, sequenceName, sampleTimes, corners; min_x = 0, max_x = 199, min_y = 0, max_y = 199, err = 0.05, lerp = false)
	#=
	...
	for (n,t) in ProgressBar(enumerate(sampleTimes))
		fillObservations!(obs, stateSplines, t, corners; min_x = min_x, max_x = max_x, min_y = min_y, max_y = max_y, err = err, lerp = lerp)
